...
const initAIAssistant = () => {
    ...
    if (!chatInput || !sendBtn || !chatHistory || !modelSelect) return;

    // Chat history state
    let messages = [
        { role: "system", content: "You are the ORBITAL AI Assistant. You are a highly advanced, space-science-focused AI. Keep your answers concise, accurate, and themed around space, telemetry, NASA data, and cosmos exploration. Do not use markdown if possible, keep it plain text friendly for a cyber terminal." }
    ];

    const appendMessage = (role, text) => {
        const msgDiv = document.createElement('div');
        msgDiv.className = `ai-message ${role === 'user' ? 'ai-user' : 'ai-system'}`;
        
        const avatar = document.createElement('div');
        avatar.className = 'ai-avatar';
        avatar.innerHTML = `<span class="material-symbols-outlined">${role === 'user' ? 'person' : 'rocket_launch'}</span>`;
    ...
    const sendMessage = async () => {
        ...
        try {
            const isLocal = modelSelect.value === 'local';
            let aiResponse = "";
            
            if (isLocal) {
                // Call local Raspberry Pi llama-server
                const response = await fetch('http://192.168.50.2:8080/v1/chat/completions', {
                    method: 'POST',
                ...
                if (!response.ok) throw new Error(`Local Node responded with ${response.status}`);
                
                const data = await response.json();
                aiResponse = data.choices[0].message.content;
                
            } else {
                // Call Free Online LLM (Pollinations Text API)
                ...
                if (!response.ok) throw new Error(`Cloud Node responded with ${response.status}`);
            ...
            const errMsg = modelSelect.value === 'local' 
                ? "SYSTEM ERROR: Unable to establish connection with Raspberry Pi node at 192.168.50.2. Ensure the node is powered on, Ethernet is connected, and llama-server is running." 
                : "SYSTEM ERROR: Cloud API request failed. Check your internet connection.";
